  ...
  class SocialLinksTag < Liquid::Tag
    ...
    def render(context)
      ...
      socials.map do |social|
        ...
        social_title = SOCIAL_TITLES.fetch(social[0], social[0].gsub('_', ' ').capitalize) #
          ...
          if social[1].is_a?(Hash) && social[1]['logo']
            ...
            if image_extensions.include?(file_ext)
              ...
              if file_ext == 'svg'
                ...
                if logo_value.include?('://')
                  img_code = "<img src='#{logo_value}' alt='#{social_title}'>"
                else
                  img_code = "<img src='#{relative_url(logo_value, context)}' alt='#{social_title}'>"
                end
              end
              "<a href='#{url}' title='#{social_title}'>#{img_code}</a>"
            else
              # It's an icon class string, render as icon (from any font source)
              icon_html = "<i class='#{logo_value}'></i>"
              "<a href='#{url}' title='#{social_title}'>#{icon_html}</a>"
            end
          else
            ...
            "<a href='#{url}' title='#{social_title}'>#{icon}</a>"
          end
        else
